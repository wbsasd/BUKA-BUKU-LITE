...
<div class="card">
  <div class="card-body">
    <h5 class="card-title mb-3">Tambah Borrowing</h5>

    <form method="POST" action="{{ route('admin.borrowings.store') }}" class="row g-3">
      @csrf
      ...
      <div class="col-md-6">
        ...
        <select name="book_id" class="form-select @error('book_id') is-invalid @enderror">
          @foreach($books ?? [] as $bk)
            <option value="{{ $bk->id }}" @selected(old('book_id')==$bk->id)>{{ $bk->title }}</option>
          @endforeach
        </select>
